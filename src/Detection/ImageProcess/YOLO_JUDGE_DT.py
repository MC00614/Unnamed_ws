...
def YOLO_JUDGE_DT(img, roi_box, SLlocation):
    ...
    # cadiBox없으면 욜로 안돌림
    if len(roi_box)==0 and 1<SLlocation:
    ...
    for out in outs:
        for detection in out:
            ...
            if confidence > 0.5 and class_id <= 11:
                ...
                elif 1<SLlocation and (width//2)<x and y<(height*0.6):
                    box.append((x, y, w, h))
                    confidences.append(float(confidence))
                    class_ids.append(class_id)
